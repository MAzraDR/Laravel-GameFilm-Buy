...
<body class="bg-light">
    <main class="container">
        ...
        <form method="post" action="{{ route('gamesserver.store') }}">
            @csrf
            <div class="mb-3 row">
                ...
                <tbody>
                    ...
                    @foreach($games as $games)
                        <tr>
                            ...
                            <td>{{ $games->year }}</td>
                            <td>{{ $games->rating }}</td>
                            <td>{{ $games->sinopsis }}</td>                                                        
                            <td>
                                <a href='{{ url("/games/{$games->id}/edit") }}' class="btn btn-warning btn-sm">Edit</a>
                                <form action="{{ url("/games/{$games->id}") }}" method="post" style="display: inline-block;">
                                    @csrf
                                    @method('delete')
